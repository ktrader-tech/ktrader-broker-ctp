...
object CtpBrokerInfo: ApiInfo {
    ...
    val configKeys: List<Pair<String, String>> = listOf(
        ...
        Pair("CachePath", "String 存贮订阅信息文件等临时文件的目录，在 KTrader 中不生效。默认为 ./data/ctp/"),
        Pair("Timeout", "Long 接口调用超时时间（单位：毫秒）。默认为 6000"),
        Pair("DisableAutoSubscribe", "Boolean 是否禁止自动订阅持仓合约的行情（用于计算合约今仓保证金以及查询持仓时返回最新价及盈亏）。默认为 false"),
        Pair("DisableFeeCalculation", "Boolean 是否禁止计算保证金及手续费（首次计算某个合约的费用时，可能会查询该合约的最新 Tick、保证金率、手续费率，造成额外开销，后续再次计算时则会使用上次查询的结果）。默认为 false"),
    )
}