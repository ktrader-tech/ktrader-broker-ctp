...
object CtpBrokerInfo: ApiInfo {
    ...
    override val description: String = """
        ...
        * 支持 7x24 小时全自动运行
        ...
        支持的额外参数：
        querySecurity/queryLastTick：[extras.ensureFullInfo: Boolean = true]【是否确保信息完整（保证金费率、手续费率、当日价格信息（涨跌停价、昨收昨结昨仓）），如果之前没查过，会耗时】
        querySecurities/queryAllSecurities/queryOptions：[extras.ensureFullInfo: Boolean = false]【是否确保信息完整（保证金费率、手续费率、当日价格信息（涨跌停价、昨收昨结昨仓）），如果之前没查过，会耗时。当 useCache 为 false 时无效，且返回的 SecurityInfo 信息不完整】
    """.trimIndent()
    ...
    val configKeys: List<Pair<String, String>> = mutableListOf(
        ...
        Pair("StatusTickDelay", "Long 补发纯状态 Tick 的延迟时间（单位：毫秒）。默认为 900")
    ).run {
